package np.com.rabin.restaurant.common;
...
public class EndpointErrorHandler {
    ...
    public ResponseEntity<ErrorInfo> handleRestaurantNotFoundException(HttpServletRequest request, RestaurantNotFoundException ex, Locale locale) {
    ...
    public ResponseEntity<ErrorInfo> handleInvalidRestaurantException(HttpServletRequest request,
        ...
        response.setUrl(request.getRequestURL().toString());
        response.setMessage(messageSource.getMessage(ex.getMessage(), ex.getArgs(), locale));
        return new ResponseEntity<ErrorInfo>(response, HttpStatus.NOT_ACCEPTABLE);
    }

    @ExceptionHandler(Exception.class)
    public ResponseEntity<ErrorInfo> handleException(Exception ex, Locale locale) {
        String errorMessage = messageSource.getMessage(UNEXPECTED_ERROR, null, locale);
        return new ResponseEntity<ErrorInfo>(new ErrorInfo(errorMessage), HttpStatus.INTERNAL_SERVER_ERROR);
    }
}
